...
export class AuthMiddleware {
    static async validateJWT(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!authorization) throw CustomError.forbidden('Sin token de autorización')
            if (!authorization.startsWith('Bearer ')) throw CustomError.forbidden('Formato de token inválido')

            const token = authorization.split(' ').at(1) ?? ''
            const payload = await JwtAdapter.validateToken<{ id: string }>(token)
            if (!payload) throw CustomError.forbidden('Token inválido')

            const user = await UserModel.findById(payload.id)
            if (!user) throw CustomError.forbidden('Usuario invalido')
    ...
    static async validateAdmin(req: Request, res: Response, next: NextFunction) {
        if (!req.body.userData) throw CustomError.forbidden('Unauthorized')
